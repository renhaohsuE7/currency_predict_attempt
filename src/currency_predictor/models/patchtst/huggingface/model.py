...
class PatchTSTHuggingFace(TransformerBasedModel):
        ...
        **kwargs,
        ...
        if config is not None:
            ...
            if (
                ...
                and config.pretrained_model_name_or_path
            ):
                self.pretrained_model_name_or_path = (
                    config.pretrained_model_name_or_path
                )
            if (
                fine_tune_mode == "from_scratch"
                and config.fine_tune_mode != "from_scratch"
            ):
    ...
    def _prepare_data(
        self, X: pd.DataFrame, y: Optional[pd.Series] = None, fit_scaler: bool = True
        ...
        past_values, future_values = self._create_sequences(
            values_scaled, self.context_length, self.prediction_length
        )
    ...
    def fit(
        ...
        **kwargs,
        ...
        learning_rate = (
            learning_rate if learning_rate is not None else effective.learning_rate
        )
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if self.use_multi_channel and predictions.shape[-1] > 1:
                    ...
                    mean_pred = (
                        predictions.squeeze(0)[:, self._target_channel_idx]
                        .cpu()
                        .numpy()
                    )
            ...
            with torch.no_grad():
                ...
                for _ in range(n_samples):
                    with torch.no_grad():
                        ...
                        s = (
                            sample_out.prediction_outputs.squeeze(0)
                            .squeeze(-1)
                            .cpu()
                            .numpy()
                        )
    ...
    def load_model(self, filepath: str) -> bool:
        ...
        try:
            ...
            self.pretrained_model_name_or_path = metadata.get(
